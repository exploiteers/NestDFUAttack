...
#define CUR_ENV_SIZE ENVSIZE(dev_current)
...
{
	ulong rc = CUR_ENV_SIZE - sizeof (long);
...
int fw_env_write(char *name, char *value)
{
	...
	/*
	 * Overflow when:
	 * "name" + "=" + "val" +"\0\0"  > CUR_ENV_SIZE - (env-environment)
	 */
	len = strlen (name) + 2;
...
static int flash_set_bad_block (int fd, uint8_t mtd_type, loff_t *blockstart)
{
    int ret = 0;

	if (mtd_type == MTD_NANDFLASH) {
		ret = ioctl (fd, MEMSETBADBLOCK, blockstart);

		if (ret < 0) {
			perror ("Cannot set bad block mark");
		}
	}
			    ...
			    off_t offset, uint8_t mtd_type)
{
	...
	do {
		...
		bad_block = flash_bad_block (fd, mtd_type, &blockstart);
		if (bad_block < 0)		/* block test failed */
			return -1;

		if (bad_block) {		/* block is bad */
			rc = -1;
			blockstart += blocklen;
			continue;
		}
		...
		if (mtd_type != MTD_DATAFLASH) {
			if (ioctl (fd, MEMERASE, &erase) != 0) {
				fprintf (stderr, "MTD erase error on %s: %s\n",
					 ...
					 strerror (errno));
				rc = -1;
			} else if (lseek (fd, blockstart, SEEK_SET) == -1) {
				fprintf (stderr,
					 "Seek error on %s: %s\n",
					 DEVNAME (dev), strerror (errno));
				return -1;
		...
		if (rc < 0) {
			/* If failed, either retry or mark the block as bad
			 * and move on.
			 */
			if (try_count < (CONFIG_ENV_TRY_COUNT - 1)) {
				try_count++;
			} else {
				if (flash_set_bad_block(fd, mtd_type, &blockstart)) {
					return -1;
				}
				try_count = 0;
				block_seek = 0;
	...
	} while ( (processed < write_total) && (rc < 0) );
...
static int flash_write (int fd_current, int fd_target, int dev_target)
{
	...
	rc = flash_write_buf (dev_target, fd_target, environment.image,
			      CUR_ENV_SIZE, DEVOFFSET (dev_target),
			      DEVTYPE(dev_target));
	if (rc < 0)
		return rc;
...
static int flash_read (int fd)
{
	...
	rc = flash_read_buf (dev_current, fd, environment.image, CUR_ENV_SIZE,
			     DEVOFFSET (dev_current), mtdinfo.type);

	return (rc != CUR_ENV_SIZE) ? -1 : 0;
}

static int flash_io (int mode)
{
	int fd_current, fd_redundant, rc, dev_redundant;
	...
	if (HaveRedundEnv) {
		...
		dev_redundant = !dev_current;
		/* dev_target: fd_target, erase_target */
		fd_redundant = open (DEVNAME (dev_redundant), mode);
		if (fd_redundant < 0) {
			fprintf (stderr,
				 "Can't open %s: %s\n",
				 DEVNAME (dev_redundant),
				 strerror (errno));
			rc = -1;
			goto exit;
		}
		...
		dev_redundant = dev_current;
		fd_redundant = fd_current;
	}

	if (mode == O_RDWR) {
		rc = flash_write (fd_current, fd_redundant, dev_redundant);

		if (HaveRedundEnv && (rc < 0)) {
			/* If failed, try current */
			rc = flash_write(fd_current, fd_current, dev_current); 
		}
	} else {
		rc = flash_read (fd_current);
                
		/* If failed, try redundant */
		if (HaveRedundEnv && (rc < 0)) {
			rc = flash_read (fd_redundant);
		}
	}

	if (HaveRedundEnv) {
		if (close (fd_redundant)) { 
			fprintf (stderr,
				"I/O error on %s: %s\n",
				DEVNAME (dev_redundant),
				strerror (errno));
