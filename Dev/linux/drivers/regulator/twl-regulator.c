...
static int twlreg_set_mode(struct regulator_dev *rdev, unsigned mode)
{
	...
	default:
        ...
        /* 
	 * I'm removing this check.  For our PMIC, setting the group enables
	 * the regulator, and if we don't want the default voltage, we don't
	 * want it to turn on until we've set it.  Since core.c sets the voltage
	 * before the mode, we need to be able to set the mode when the group is
	 * unassigned. (JS)
	 */
#if 0
