...
{
	struct omap_dss_device *device = data;
	struct manager_cache_data *mc;
	struct overlay_cache_data *oc;
	const int num_ovls = dss_feat_get_num_ovls();
	const int num_mgrs = dss_feat_get_num_mgrs();
	...
	omap_dispc_unregister_isr(dss_apply_irq_handler, NULL, dss_cache.irq_mask);

	dss_cache.irq_enabled = false;
        (void*) device;  //Unused - quiet compiler
end:
...
static int omap_dss_mgr_apply(struct omap_overlay_manager *mgr)
{
	...
	/* Based on manager configure irq_mask */
	if (mgr && (mgr->id == OMAP_DSS_CHANNEL_DIGIT) &&
			 (mgr->supported_displays | OMAP_DISPLAY_TYPE_VENC))
		dss_cache.irq_mask = DISPC_IRQ_EVSYNC_ODD | DISPC_IRQ_EVSYNC_EVEN;
	else
		dss_cache.irq_mask = DISPC_IRQ_VSYNC;
